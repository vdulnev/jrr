...
// ─── SUB-SCREEN HEADER ───────────────────────────────────────
function SubScreenHeader({ title, subtitle, onBack, right, inLibrary=false }) {
  return (
    <div style={{ padding: inLibrary ? '12px 20px 16px' : '28px 20px 16px', flexShrink:0 }}>
      {!inLibrary && (
    ...
    <div style={{ borderBottom:`1px solid ${T.line}` }}>
      <div style={{ display:'flex', alignItems:'center', gap:14, padding:'12px 20px', cursor:'pointer' }}
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ fontSize:14, fontWeight:500, color:T.text }}>{[album.year, album.title].filter(Boolean).join(' - ')}</div>
          <div style={{ fontSize:11, color:T.text3, marginTop:2 }}>{album.artist}</div>
        </div>
